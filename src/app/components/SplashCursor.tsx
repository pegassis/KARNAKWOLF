...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    // Adjust parameters for mobile devices: increase splat "thickness" and color intensity
    const isMobile = typeof navigator !== 'undefined' &&
      (/Mobi|Android|iPhone|iPad|iPod/i.test(navigator.userAgent) ||
        (window.matchMedia && window.matchMedia('(pointer: coarse)').matches));

    const mobileRadiusMultiplier = isMobile ? 1.6 : 1.0;
    const mobileForceMultiplier = isMobile ? 1.25 : 1.0;
    const mobileColorBoost = isMobile ? 1.25 : 1.0;

    const adjSplatRadius = Math.max(0.01, SPLAT_RADIUS! * mobileRadiusMultiplier);
    const adjSplatForce = Math.max(1, SPLAT_FORCE! * mobileForceMultiplier);
    const adjBackColor = {
      r: Math.min(1, BACK_COLOR.r * mobileColorBoost),
      g: Math.min(1, BACK_COLOR.g * mobileColorBoost),
      b: Math.min(1, BACK_COLOR.b * mobileColorBoost)
    };
    ...
    let config = {
      ...
      CURL: CURL!,
      SPLAT_RADIUS: adjSplatRadius,
      SPLAT_FORCE: adjSplatForce,
      SHADING,
      COLOR_UPDATE_SPEED: COLOR_UPDATE_SPEED!,
      PAUSED: false,
      BACK_COLOR: adjBackColor,
      TRANSPARENT
    };
